...
int
create_screen(struct Screen* screen, int w, int h)
{
...
int
free_screen(struct Screen* screen)
{
  free(screen->buffer);
...
void
show_cursor(struct Screen* screen, BOOL state)
{
...
int
get_alignment_offset(const char* str, int align)
{
  if(align == ALIGN_LEFT)
  {
    return 0;
...
int
print_string(struct Screen* screen, const char* str, WORD attr, int x, int y, int align)
{
...
void
print_console(struct Screen* screen)
{
  COORD c_pos = { 0, 0 };
