...
get_tile_temp(int tile)
{
  switch(tile)
  {
    ...
    case TILE_FLOOR: return 15;
    default: return 10;
  }
};

WORD
get_temp_attributes(int temp)
{
  if(temp < 0)
  {
    return FG_CYAN;
...
void
init_status(struct Status* status, int hp, int hunger, int thirst)
{
  status->hp = hp;
  status->max_hp = hp;
